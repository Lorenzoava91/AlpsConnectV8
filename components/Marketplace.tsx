...
const MapRevalidator = () => {
  const map = useMap();
  useEffect(() => {
    // FIX: Disable tap delay on mobile directly via Leaflet options
    // instead of passing untyped prop to MapContainer
...
const TripMarker = React.memo(({ trip, t, onDetail, onBook, onGuideClick }: { trip: Trip, t: any, onDetail: () => void, onBook: () => void, onGuideClick: (e: any) => void }) => {
  ...
  return (
    <Marker 
        ...
        icon={icon}
    >
        {/* FIX: Removed className from Popup prop to avoid TS errors. Moved class to inner div. */}
        <Popup closeButton={false} maxWidth={300} minWidth={280}>
           {/* Add pointer-events-auto to ensure clicks register inside popup */}
           <div className="custom-popup p-0 pointer-events-auto">
...
const Marketplace: React.FC<Props> = ({ trips, onRequestJoin, lang }) => {
  ...
  return (
    <div className="space-y-6 md:space-y-8 pb-20">
      ...
      {/* FILTERS BAR */}
      <div className="sticky top-4 z-[1050] space-y-3 pointer-events-none">
      ...
      {viewMode === 'map' && (
        <div className="bg-slate-100 rounded-3xl overflow-hidden h-[600px] relative border border-slate-200 shadow-inner">
            <MapContainer 
                ...
                // FIX: Removed 'tap={false}' prop to fix build error. Replaced with MapRevalidator logic.
                // tap={false} // REMOVED
            >
                <MapRevalidator />
                <TileLayer
