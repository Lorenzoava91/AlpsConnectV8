...
const Dashboard: React.FC<Props> = ({ trips, onAddTrip, onApproveRequest, guide, chats, lang }) => {
  ...
  // Local state for profile avatar to allow instant update in demo without global refactor
  const [previewAvatar, setPreviewAvatar] = useState(guide.avatar);
    ...
    <div className="space-y-6 md:space-y-8 pb-20">
            ...
            <div className="bg-white rounded-3xl shadow-sm border border-slate-100 overflow-hidden">
                <div className="p-4 md:p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50">
                    ...
                    <button 
                        ...
                        className={`bg-slate-900 text-white px-3 py-2 md:px-4 md:py-2.5 rounded-xl font-bold text-xs md:text-sm hover:bg-slate-800 transition-colors shadow-lg shadow-slate-200 items-center gap-2 ${activityFilter !== 'all' ? 'hidden md:flex' : 'flex'}`}
                    >
      ...
      {activeTab === 'chat' && (
        <div className="animate-in fade-in duration-300">
           <ChatInterface 
              conversations={chats} 
              currentUserAvatar={previewAvatar}
              lang={lang} 
           />
        </div>
      )}
             ...
             <div className="bg-white rounded-3xl shadow-sm border border-slate-100 overflow-hidden">
                 ...
                 <form onSubmit={handleProfileUpdate} className="p-6 md:p-8 space-y-6">
                     <div className="flex items-center gap-6 mb-6">
                         ...
                         />
                         <div 
                            className="w-20 h-20 rounded-full bg-slate-100 overflow-hidden shrink-0 relative group cursor-pointer border-2 border-transparent hover:border-slate-300 transition-all"
