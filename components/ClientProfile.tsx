...
import { MapPin, Calendar, Camera, Activity, Award, Link as LinkIcon, Smartphone, Mountain, Settings, Plus, User, Shield, CreditCard, Save, MessageSquare, Quote, Wallet, Clock, ArrowDownLeft, ArrowUpRight, CheckCircle2, Star, Image as ImageIcon } from 'lucide-react';
...
const initialActivities: ActivityLog[] = [
  { id: 1, title: 'Gran Paradiso Vetta', date: '2023-07-15', guideName: 'Mario Rossi', location: 'Valsavarenche', type: 'guided', stats: '1300m d+', photos: ['https://images.unsplash.com/photo-1464822759023-fed622ff2c3b?q=80&w=300&auto=format&fit=crop'] },
  { id: 2, title: 'Corso Ghiaccio Base', date: '2023-02-02', guideName: 'Luca Bianchi', location: 'Cogne', type: 'guided', photos: [] },
  { id: 3, title: 'Allenamento Sella', date: '2023-11-10', location: 'Dolomiti', type: 'personal', stats: '800m d+', photos: [] },
];

const ClientProfile: React.FC<Props> = ({ client, chats, lang }) => {
  const [isStravaConnected, setIsStravaConnected] = useState(false);
  const [activeTab, setActiveTab] = useState<'journal' | 'billing' | 'chat' | 'reviews' | 'payments'>('journal');
  ...
  const t = {
    it: {
        tabs: { journal: "Diario", payments: "Spese", reviews: "Feedback", billing: "Dati", chat: "Chat" },
        level: "Livello",
        feedback: "Feedback",
        ...
        stravaDesc: "Sincronizza Strava per tenere traccia dei dislivelli e migliorare il tuo Profilo Esperienza.",
        connectStrava: "Connetti Strava",
    ...
    en: {
        tabs: { journal: "Journal", payments: "Expenses", reviews: "Feedback", billing: "Info", chat: "Chat" },
        level: "Level",
        feedback: "Feedback",
        ...
        stravaDesc: "Sync Strava to track elevation gain and improve your Experience Profile.",
  ...
  const handlePhotoUpload = (activityId: number | string, e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files[0]) {
      ...
      setActivities(prev => prev.map(act => {
        if (act.id === activityId) {
          return { ...act, photos: [...(act.photos || []), imageUrl] };
        }
        return act;
      }));
    }
      ...
      <div className="max-w-5xl mx-auto px-4 md:px-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
        ...
        {activeTab === 'journal' && (
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="lg:col-span-2 space-y-6">
              ...
              <div className="space-y-6">
                {activities.map(activity => (
                  <div key={activity.id} className="group bg-white p-4 md:p-5 rounded-3xl border border-slate-100 shadow-sm hover:shadow-md transition-all">
                    <div className="flex items-start gap-4">
                        <div className={`p-2 md:p-3 rounded-2xl shrink-0 ${activity.type === 'guided' ? 'bg-indigo-50 text-indigo-600' : 'bg-amber-50 text-amber-600'}`}>
                    ...
                    <div className="mt-4 pt-4 border-t border-slate-50">
                        <div className="flex gap-2 overflow-x-auto pb-2 no-scrollbar">
                            {activity.photos && activity.photos.map((photo, idx) => (
                                <div key={idx} className="w-20 h-20 md:w-24 md:h-24 rounded-xl overflow-hidden shrink-0 border border-slate-100 cursor-pointer">
                                    <img src={photo} className="w-full h-full object-cover" alt="activity" />
                                </div>
                            ))}
                            
                            {/* Upload Button */}
                            <label className="w-20 h-20 md:w-24 md:h-24 rounded-xl border-2 border-dashed border-slate-200 flex flex-col items-center justify-center text-slate-400 hover:text-slate-600 hover:border-slate-300 hover:bg-slate-50 transition-all cursor-pointer shrink-0">
                                <Camera size={20} />
                                <span className="text-[9px] font-bold uppercase mt-1">{t.addPhoto}</span>
                                <input 
                                    type="file" 
                                    accept="image/*" 
                                    className="hidden" 
                                    onChange={(e) => handlePhotoUpload(activity.id, e)}
                                />
                            </label>
                        </div>
                    </div>
            ...
            {/* Right Col: Sync & Badges */}
            <div className="space-y-6">
               
               {/* STRAVA SYNC CARD (Moved here) */}
               <div className="bg-orange-50 p-6 rounded-3xl border border-orange-100">
                   <div className="flex justify-between items-start mb-4">
                        <div className="w-10 h-10 bg-orange-100 text-orange-600 rounded-xl flex items-center justify-center">
                            <Activity size={20} />
                        </div>
                        {isStravaConnected && <CheckCircle2 size={20} className="text-orange-600" />}
                   </div>
                   <h3 className="font-bold text-lg text-orange-900 mb-2">{t.sync}</h3>
                   <p className="text-orange-800/70 text-xs mb-6 leading-relaxed">
                       {t.stravaDesc}
                   </p>
                   ...
                   {!isStravaConnected ? (
                       <button 
                         ...
                         className="w-full py-2.5 bg-orange-500 hover:bg-orange-600 text-white font-bold text-sm rounded-xl transition-colors shadow-lg shadow-orange-200 flex items-center justify-center gap-2"
                       >
                           <LinkIcon size={16} /> {t.connectStrava}
                       </button>
                   ) : (
                       <div className="bg-white p-3 rounded-xl border border-orange-100 flex items-center justify-between">
                           <span className="font-bold text-xs text-orange-600 flex items-center gap-2">
                               {t.syncActive}
                           </span>
                           <button onClick={() => setIsStravaConnected(false)} className="text-[10px] font-bold text-slate-400 hover:text-slate-600 uppercase">Disconnetti</button>
                       </div>
                   )}
               </div>
