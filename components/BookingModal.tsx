...
const BookingModal: React.FC<Props> = ({ trip, onClose, onConfirm, lang = 'it' }) => {
  ...
  // Initialize calendar at the start of availability if it's in the future relative to today
  // helping user find dates immediately without scrolling
  const [currentMonth, setCurrentMonth] = useState(() => {
     return isBefore(today, tripStart) && !isSameMonth(today, tripStart) ? tripStart : new Date();
  });
  ...
  return (
    <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[1200] flex items-center justify-center p-4 animate-in fade-in duration-200">
