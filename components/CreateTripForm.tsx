...
import { Sparkles, MapPin, DollarSign, Loader2, Clock, Map as MapIcon, Users, Image as ImageIcon, Upload, X } from 'lucide-react';
...
const CreateTripForm: React.FC<Props> = ({ onSave, onCancel, lang = 'it' }) => {
  ...
  const [formData, setFormData] = useState<Partial<Trip> & { customImage?: string }>({
    title: '',
    location: '',
  ...
  const t = {
    it: {
        ...
        placeLoc: "Zona",
        uploadImg: "Carica Foto Copertina",
        uploadHint: "JPG o PNG. Max 5MB.",
        removeImg: "Rimuovi foto"
    },
    en: {
        ...
        placeLoc: "Area",
        uploadImg: "Upload Cover Photo",
        uploadHint: "JPG or PNG. Max 5MB.",
        removeImg: "Remove photo"
    }
  }[lang];
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const newTrip: Trip = {
      ...
      guideId: 'g1', // Linked to the mocked logged-in guide
      guideName: 'Jean-Pierre Luc', // Mocked logged-in guide name
      ...
      enrolledClients: [],
      pendingRequests: [],
      // Use custom image if uploaded, otherwise fallback to placeholder
      image: formData.customImage || `https://picsum.photos/seed/${formData.location}/800/600`,
      status: 'upcoming',
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          ...
          <div className="space-y-4 flex flex-col">
            ...
            <div>
               ...
               />
               <div 
                 onClick={() => !formData.customImage && fileInputRef.current?.click()}
                 className={`w-full h-32 rounded-xl border-2 border-dashed flex flex-col items-center justify-center cursor-pointer transition-all relative overflow-hidden group
                   ${formData.customImage ? 'border-slate-200' : 'border-slate-300 hover:border-slate-400 hover:bg-slate-50'}
                 `}
               >
                  {formData.customImage ? (
                    <>
                      ...
                      <div className="absolute inset-0 bg-black/40 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                         <button 
                           onClick={(e) => { e.stopPropagation(); setFormData(prev => ({...prev, customImage: ''})) }}
                    ...
                    <>
                      ...
                      <span className="text-xs font-bold text-slate-500 uppercase">{t.uploadImg}</span>
                      <span className="text-[10px] text-slate-400 mt-1">{t.uploadHint}</span>
                    </>
                  )}
               </div>
            </div>
            ...
            <div>
               ...
               <div className="h-40 w-full rounded-xl overflow-hidden border border-slate-300 shadow-inner">
                  <MapContainer 
            ...
            <div className="flex-1 flex flex-col">
              ...
              <textarea 
                required
                className="w-full px-4 py-2 rounded-lg border border-slate-300 focus:ring-2 focus:ring-slate-500 outline-none resize-none text-sm leading-relaxed flex-1 min-h-[100px]"
            ...
            <div>
               ...
               <div className="bg-slate-50 rounded-lg border border-slate-200 p-3 h-20 overflow-y-auto text-sm text-slate-600">
